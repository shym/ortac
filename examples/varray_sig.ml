...
module type S = sig
  ...
  (** valid is a helper function to check that an index is valid (i.e in [O..lenght s]) *)
  (*@ function valid (i : integer) (s : 'a sequence) : bool =
  ...
  *)
  (*@ insert_at t i x
      checks valid i t.contents
      modifies t.contents
      ensures t.contents = old (if valid i t.contents
  ...
  *)
  (*@ r = sub t i n
      checks 0 <= i < Sequence.length t.contents /\ i <= i + n - 1 < Sequence.length t.contents
      ensures r.contents = t.contents[i..i+n-1] *)
  ...
  *)
  (*@ fill t pos len x
      ...
      modifies t.contents
      ensures t.contents = Sequence.init (Sequence.length (old t.contents)) (fun i -> if i < pos \/ i >= pos + len then (old t.contents)[i] else proj x) *)
  ...
  *)
  (*@ blit src src_pos dst dst_pos len
      checks 0 <= src_pos < Sequence.length src.contents /\ src_pos < src_pos + len < Sequence.length src.contents
      checks 0 <= dst_pos < Sequence.length dst.contents /\ dst_pos < dst_pos + len < Sequence.length dst.contents
